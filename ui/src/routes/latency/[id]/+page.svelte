<script lang="ts">
    ...
    import type { LatencyRule } from "$lib/types";

    let rule: LatencyRule | null = $state(null);
    ...
    async function fetchRule() {
        try {
            const res = await fetch(`/api/latency-rules/${id}`);
            if (res.ok) {
                rule = await res.json();
                if (rule) {
                    ...
                    if (delay.type === "fixed") {
                        delayType = "fixed";
                        fixedDelay = delay.delay_ms;
                    } else if (delay.type === "random") {
                        delayType = "random";
                        randomMin = delay.min_ms;
                        randomMax = delay.max_ms;
                    } else if (delay.type === "normal") {
                        delayType = "normal";
                        normalMean = delay.mean_ms;
                        normalStdDev = delay.std_dev_ms;
                    } else if (delay.type === "spike") {
                        delayType = "spike";
                        spikeBase = delay.base_delay_ms;
                        spikeDelay = delay.spike_delay_ms;
                        spikeProbability = delay.spike_probability;
                    }
                }
            } else {
                error = "Latency rule not found";
            }
        } catch (e) {
            error = "Failed to load latency rule";
        } finally {
            loading = false;
        }
    }
    ...
    async function save() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                error = "Failed to update latency rule";
            }
        } catch (e) {
            error = "Failed to update latency rule: " + e;
        } finally {
            saving = false;
        }
    }
...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-3xl font-bold">Edit Latency Injection Rule</h1>
    </div>
                ...
                {#if delayType === "random"}
                    ...
                    >
                        ...
                        <p class="text-sm text-purple-800">
                            Each request will be delayed by a random value
                            between <strong>{randomMin}ms</strong> and
                            <strong>{randomMax}ms</strong>.
                        </p>
                    </div>
                {/if}
                ...
                {#if delayType === "normal"}
                    ...
                    >
                        ...
                        <p class="text-sm text-green-800">
                            ...
                            <strong>{normalStdDev}ms</strong>. Most delays will
                            be between {Math.max(
                                0,
                                normalMean - normalStdDev,
                            )}ms and {normalMean + normalStdDev}ms.
                        </p>
                    </div>
                {/if}
                ...
                {#if delayType === "spike"}
                    ...
                    >
                        ...
                        <p class="text-sm text-orange-800">
                            ...
                            delay, but
                            <strong
                                >{(spikeProbability * 100).toFixed(1)}%</strong
                            >
